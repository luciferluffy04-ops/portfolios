        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm text-gray-400 mb-6 max-w-xs">
              Create your first portfolio in minutes. Upload your resume and we'll do the rest.
            </p>
            <Link href="/builder"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {portfolios.map(p => (
              <div key={p.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:border-gray-300 transition-colors">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-sm font-semibold text-gray-800">{p.name || 'Untitled'}</h3>
                      <p className="text-xs text-gray-400 mt-0.5 capitalize">{p.role} · {p.template}</p>
                    </div>
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                      p.published ? 'bg-teal-50 text-teal-600 border border-teal-100' : 'bg-gray-100 text-gray-400'
                    }`}>
